"""
Seed script: parse Physical Activity Tracker.docx and insert into fitness_logs.

Usage:
    python scripts/seed_fitness.py --file "path/to/Physical Activity Tracker.docx" --user-id 1

Line format expected (all other lines are ignored):
    DD/MM/YY - Activity1, Activity2; weight_kg
"""
...
from app.models.user import User  # noqa: F401 — ensures users table is known

LINE_PATTERN = re.compile(
    r"^(\d{2}/\d{2}/\d{2})\s*-\s*(.+?);\s*(\d+\.?\d*)$"
)
...
def parse_line(raw: str) -> dict | None:
    ...
    date_str, activities_str, weight_str = m.group(1), m.group(2), m.group(3)
    ...
    # treat "Rest" as an empty activity list
    if len(activities) == 1 and activities[0].lower() == "rest":
        activities = []
...
def extract_lines(docx_path: str) -> list[str]:
    ...
    for para in doc.paragraphs:
        ...
        if text:
            lines.append(text)
    # also check tables inside the doc
...
async def seed(docx_path: str, user_id: int, dry_run: bool) -> None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        inserted = skipped = 0
        for e in entries:
            # skip if a log for this date already exists for this user
            exists = await db.execute(
                select(FitnessLog).where(
                    FitnessLog.user_id == user_id,
                    FitnessLog.date == e["date"],
                )
            )
            ...
            db.add(FitnessLog(user_id=user_id, **e))
            inserted += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed fitness_logs from a Word doc.")
